...
function avoidCollision(obj, target) {
    if (target) {
        // is collision test
        while (u1.isCollision(obj, target)) {
            if (obj.start.x > 0) {
                obj.start.y--;
...
function getPieTextMaxLength(series) {
    ...
    var maxLength = 0;
    series.forEach(function (item) {
        var text = item.format ? item.format(+item._proportion_.toFixed(2)) : u1.toFixed(item._proportion_ * 100) + '%';
...
function drawRadarLabel(angleList, radius, centerPosition, opts, config, context) {
    ...
    angleList.forEach(function (angle, index) {
        ...
        if (u1.approximatelyEqual(pos.x, 0)) {
...
function drawPieText(series, opts, config, context, radius, center) {
    ...
    seriesConvert.forEach(function (item) {
        ...
        if (lastTextObject && u1.isSameXCoordinateArea(lastTextObject.start, { x: orginX3 })) {
            if (orginX3 > 0) {
